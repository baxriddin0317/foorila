...
const SwitchThemeButton = () => {
  ...
  return (
    <div className="flex items-center gap-1.5">
      {/* Toggle Switch */}
      <button
        className={`relative inline-flex h-3.5 w-7 items-center rounded-full transition-colors focus:outline-none cursor-pointer ${isDark ? 'bg-brand-blue' : 'bg-brand-border'}`}
      ...
      <span 
        ...
        onClick={() => setTheme(isDark  ? "light" : "dark")}
      >
        {isDark ? <svg className='w-4.25 h-4.5' xmlns="http://www.w3.org/2000/svg" width={17} height={18} viewBox="0 0 24 24" fill="none">
